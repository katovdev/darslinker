...
async function showArticlePage(articleId) {
  try {
    ...
    if (data.message === 'success') {
      ...
      // Add back button to header (only if it doesn't exist)
      const headerRight = document.querySelector('.header-content');
      const existingButton = document.querySelector('.back-to-blogs-btn-header');
...
window.goBackToBlogs = function() {
  ...
  const headerButton = document.querySelector('.back-to-blogs-btn-header');
  if (headerButton) {
    headerButton.remove();
  }
